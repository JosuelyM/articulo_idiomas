...
% 		\caption{{\bf Zipf's law of the accumulated migrant words,
% grouped by receiving language.\cpnote{cambiar la figura}} We display a frequency-rank 
% plot for all language pairs, \cpnote{Josue, esto es por año, es decir, calculas la 
% frecuencia total, o en un año, o como?} DAR MAS DETALLES CUANDO SE TENGAN DETALLES 
% Y LA FIGURA FINAL  }
% 		\label{fig.ZL_receiving}
% 	\end{adjustwidth}
% \end{figure}
...
it is clear that there are some words more important than others, see 
\fref{fig.ZL_receiving}. 
Thus care must be taken when one removes a fixed proportion of words, or 
a fixed frequency, as it can make a big difference. 
...
We study the similarity of the shape of usage \eref{}, as measured by $\left\langle D \right\rangle $ (see \eref{}), when 
some words are eliminated. The top plots correspond to the case in which the 
lower rank words are being eliminated, whereas in the lower plots we start eliminating the
higher rank words. 

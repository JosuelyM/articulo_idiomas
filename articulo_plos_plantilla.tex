...
\cgnote{Tenemos s\'olo 11 referencias, valdr\'ia la pena incluir m\'as. Se pueden obtener varias de nuestros art\'iculos previos. No digo citar por citar, pero s\'i por lo menos tener 20, mejor 30+.}
\
...
sfollowing a sigmoid curve. %logarithmically.
 \cgnote{logarithmically or ``following a sigmoid curve''? what
would be more precise?}
\jmnote{Había puesto la curva logatirmica porque al principio fue el primer ajuste que hice. Corregí el texto ya que en el texto se menciona el ajuste a la sigmoide }

\cgnote{In the next section, ***}

\cpnote{Obvio aca toca poner algunos caveats y discutir la seccion de la
estabilidad} \cgnote{o tal vez al final? digo, primero hay que presentar el
m\'etodo y despu\'es explicar sus l\'imites}

\cgnote{Uno de los caveats es que como hacemos comparaciones automatizadas de
cadenas (llam\'emosle ``blind big data''), nuestras comparaciones son puramente
de palabras completas, ya no digamos sem\'anticas. Por ejemplo, estamos
limitados para estudiar influencia de y hacia ruso, simplemente porque tiene
otro alfabeto. Hay otras transformaciones que no detectamos, e.g. en espa\~nol
se agrega `e' antes de `sp' (especial/special). Y tambi\'en hay casos donde la
influencia puede ser de otros idiomas, e.g. Lat\'in, aunque en otros casos se
puede detectar por d\'onde lleg\'o la influencia (e.g. sushi?). Otra limitante
es que nos enfocamos en palabras de uso frecuente. S\'olo decir que los
estudios comparativos de ``insightful small data'', donde humanos identifican
manualmente flujos de palabras seguir\'an siendo \'utiles y necesarios. }

\jmnote{Una de las principales limitantes del meotodo para detectar la influcencia es que se necesita que los idiomas tengan el mismo alfabeto. Deel ejemplo de sushi, el metodo mostraria que "nacio" en alguno de los idiomas que tenemos digamos el idioma A y de ahi se fue al resto; la limitante es que A es el idioma que se uso para transmitir la palabra a los demás  pero el algoritmo identifica a A como el orige, para corregir todos los posibles casos como esté se tendrian que tener más bases de datos. }
\
...
take into account. } \cgnote{no es cierto, todos los sustantivos son content words \url{https://en.wikipedia.org/wiki/Content_word} }
...
as \textit{migrant words}. \cgnote{Me acabo de enterar que se usa el t\'ermino ``loanword''. Habr\'ia que checar si nuestras migrant words son iguales (y entonces usar loanwords), o cu\'al es su diferencia (y entonces explicar la diferencia).}
...
which language are the NMW coming. \cgnote{convendr\'ia entonces distinguir los dos tipos de NMW, tambi\'en en la figura, tal vez $NMW_{in}$ y $NMW_{out}$ para cada idioma} The results are presented in 
...
during the second half of the century, after the end of World War II. \cgnote{yo no veo esta tendencia tan clara. Italiano parece que recibe m\'as antes de 1950. }
...
\cgnote{podemos notar que de entre los idiomas que comparamos, hay tambi\'en unos que importan menos palabras que otros... quisiera normalizar primero los ejes $y$, porque parecer\'ia que hay mucha influencia entre Espa\~nol e Italiano, pero tal vez es una influencia relativa, y absoluta sea comparable a influencias entre otros idiomas}¸
...
separated by the origin of the different NMW. \cgnote{en ejes $x$ mejor poner 'Decade', en eje $y$ no ser\'ia NMW? Tambi\'en sugiero que todos los ejes $y$ tengan la misma escala, e.g. de 0 a 100, para comparar mejor las dimensiones.}
% 
% Among language
...
time, Zipf's law is observed. \cgnote{convendr\'ia poner una curva ``ideal'' (tal vez punteada) en cada panel, para ver qu\'e tanto se desv\'ian los datos de la ley de Zipf.}}

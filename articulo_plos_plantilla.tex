...
\cgnote{estoy de acuerdo en que PLoS ONE ser\'ia un venue apropiado, pero hay dinero?}
...
mixed\cpnote{Creo que podemos aca hablar mas de un mixing de cultura. Irnos de
cosas generales a cosas particulares. Ahora, gracias a google ngram podemos aproximarnos
a este problema mediante el lengauje}.
...
issues related to the internet, a word without translation in Spanish\cpnote{Ojo, ver si en la RAE no están estas palabras}.
 
This trending is not only affecting \cpnote{has not only affected} Spanish,  but also other languages that are
...
publication the most used “n-grams”\cpnote{Dos cosas, la primera es que en latex 
...
y nen ls siguiente son diferentes}. The "$n$-grams" are the words or set of
...
as \textbf{migrant words}. \cpnote{Normalmente las palabras claves las pongo 
en italica. toca ver como le hace en el journal que elijamos.}
...
different set from the source language. \cpnote{Creo qe el recieving language tambien tiene
que entrar en el top, no?} \cpnote{No recuerdo porque usamos ese numero (5000). Tocará 
medio comentarlo} To determine the source language, we
...
a locality, while in Spanish, \cpnote{mayor} is an adjective to indicate that something
...
written \textit{imaginer} in French and \textit{imaginar} in Spanish. \cpnote{decir 
que entonces nuestro algoritno no la detecta.} Finally, in some cases, the
...
shows how much influence the things have\cpnote{Esta frase es muy importante, pero 
no estoy del todo de acuerdo con ella. Platicar con Josue y con CarlosG}. We built a first method to quantify
...
languages \textit{A}​ has been influential \cpnote{Mala redacción}; the second way was by taking
...
in the years around) where the event occurred.\cpnote{redaccion como rara. Quizá meteria
la definicion de semantica entre comas o algo asi menor. Quizá mencionar que 
ese concepto es el más util para agrupar las palabras.}
...
\begin{figure}[!h]
	...
	\caption{{\bf Languages ​​as sources and receivers of new words.} During the 20th century, only English language has been the language that has migrated more words than it has received. French, German, Italian and Spanish have received more words during the second half of the century, after the end of World War II.\cgnote{estandarizar ejes $y$???}}
	\label{fig.NMW_OR}
	\end{adjustwidth}
\end{figure}
